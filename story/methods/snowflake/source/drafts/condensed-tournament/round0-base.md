...
Both results outlive the state that produced them, which is why he trusts it. He files the clarity under flow and calls it the best weekend of work in his life.
...
He starts with the official rescue timeline and begins reciting it: the search attempts, the failed approaches, the temporary path that let rescuers reach her, the extraction, the attempts to reach the rest of her family.
...
He still controls the original apparatus, and he believes one thing may still be worth doing with it: a synchronized decommissioning initiated from the root of the whole replica lineage, timed with the participating sites, denying the operation its cleanest remaining closures. He dismantles the active core, the defective ring, the LN2 stage, the thermal and RF drive, the sensors, the controller, and trucks it to a remote industrial test site many miles from the house, because whatever physical danger exists should stay near the energized hardware and far from Ruth and Tess. Into the precommitted distributed response he writes one personal input: a live safety signal from the garage and the house. If it drops, every site halts and falls back to a verification run. It is a father's-side clause, protection encoded the only way he knows.
