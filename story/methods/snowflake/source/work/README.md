---
schema: causal-projector.snowflake-work/v1
activeStep: "05"
currentDocumentId: character:evan-hale:full-character-synopsis
supportingDocumentId: character:evan-hale:full-character-synopsis-planning
currentStatus: provisional
---

# Current work

This file is the single pointer to active Snowflake work. Stable indexes should
link here instead of repeating current status.

- Active Snowflake step: 05
- Current deliverable:
  [`canon/characters/evan-hale/05-full-character-synopsis.md`](../canon/characters/evan-hale/05-full-character-synopsis.md)
- Deliverable status: provisional, awaiting review
- Supporting planning:
  [`canon/characters/evan-hale/05-supporting-planning.md`](../canon/characters/evan-hale/05-supporting-planning.md)

## Immediate review decision

Evan's planning decisions are accepted. His first treatment was judged too
stilted and list-like, so it was expanded to remove orphaned references,
explain causal transitions within the treatment, and carry his relationships
without a page-length constraint. The current pass moves events and their
immediate consequences ahead of after-the-fact explanation throughout the
synopsis, except where setup is needed to understand a choice, an experiment,
or its stakes.

Does the revised treatment now read continuously, with its explanations
arriving where the reader needs them rather than before the events they
explain?

## Open-question destinations

- [Plot](open-questions/plot.md)
- [Science](open-questions/science.md)
- [Motifs](open-questions/motifs.md)
- [Naming](open-questions/naming.md)
- [Character and character-like subjects](../canon/characters/README.md)

Character-specific questions belong in `open-questions.md` inside that
character's directory. These topical files own only cross-cutting unresolved
questions.

## Noncanonical research

- [Early-source research quarry](research/README.md)

The research quarry contains compact extracts from the original conversation
and early development dossiers. It is useful for future technical and craft
decisions but has no canonical authority.
