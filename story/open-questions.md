# Open questions

Generated deterministically from the internal Snowflake question owners.

## Open questions: Esther "Tess" Kertesz


- Exact ferry name, route, date, and accident mechanics.
- Her Fairbanks guardians' names, character, and present relationship with
  Tess.
- Exact records that first contradict her survival history, including their
  timing, degree, and visibility.
- How she moves from professional audit to living in Evan and Ruth's house.
- The precise experiment in which Evan secretly uses her choices.
- Exact scene mechanics of Evan's admission, their rupture, and the renewed
  trust that begins after Ruth's death without becoming immediate forgiveness.
- Whether Tess knows Evan remembers her childhood disaster from the news before
  he tells her.
- Exact visual and spatio-acoustic symbols Evan and Tess recover after sharing
  the same impossible turn.
- Exact wording and image-to-referent mapping in the clinical DMT transcript.

### Related cross-subject questions

- [Plot questions](../../../work/open-questions/plot.md) own the Tess and Ruth
  friendship sequence and final exchange.
- [Mouse's questions](../mouse/open-questions.md) own the adoption details and
  Mouse's terminal action.

## Open questions: Evan Hale


- Exact employer, company role if applicable, and day-job problem in the
  revised version.
- Whether his former wife appears in the present timeline.
- Exact nature and timing of his prior DMT experience, and how much of it
  remains in the revised version.
- How much personal fame or public recognition Evan receives before causal
  collapse.
- His last meaningful exchange with Ruth before leaving to follow the
  proof-to-stop chain.

## Open questions: Lena Varga


- Exact university, shared-facility name, observatory name, and geography.
- Her exact final observation, sentence, action, and last coherent gesture
  before meaningful personal continuity fails in the fixed-successor collapse.
- Whether she is Hungarian, Hungarian-American, or simply has the surname
  Varga; avoid needless overlap with Tess's Hungarian surname unless it is
  intentionally explained.

## Open questions: Mouse


Locked boundary: the final portion of the genesis-operation chapter uses
Mouse's literal, animal viewpoint. The questions below concern execution, not
whether that viewpoint is used.

- Exact circumstances and timing of Tess adopting Mouse.
- Mouse's exact last-reference action, the transition into her viewpoint, and
  her sensory experience of the incoming Big Bang, including Evan and Tess's
  final words as Mouse hears without comprehending them.

## Open questions: The projector operation


- In-world technical and colloquial names for the operation.
- The exact live implementation of the proof-to-stop test, within the locked
  constraint that it closes the first distributed projection cycle without
  Evan or the original garage apparatus.
- Which labs, agencies, companies, or other institutions become later
  carriers after the scientific containment system becomes the first
  independent carrier.
- How widely the operation itself is recognized before terminal convergence.
- Exact participating facilities, site count, study administration, and
  replication schedule.
- The resource and full contingency chain in the representative cross-lab
  handoff.
- The exact stored-energy source, hardware failure, physical injury mechanics,
  site, geography, and ejecta of the local compound catastrophe.
- Whether the dead researcher and surviving close professional collaborator
  had any further personal relationship.
- How the DMT observer renders the invariant in sensory and symbolic terms.
- Whether the epilogue's reconstructed figure is best read as Evan's
  remainder, the operation, the child universe modeling its origin, or a
  relation in which those distinctions no longer apply.
- How narration preserves preconceptual, extra-spatial contact and perturbative
  sensory effects without reducing them to an ordinary scene with unfamiliar
  decorations.
- Exact replica site, apparatus condition, and route through the merged-site
  overlap.
- Whether mutual veto is the final terminal structure or only an emotional
  model.
- Exact noncommuting operations controlled by Evan and Tess, including
  game-theoretic payoff states and permit/veto combinations.
- Exact terminal hardware, waveform, protocol, and physical mechanism of
  genesis (deliberately deferred).

### Related cross-subject question

- [Mouse's questions](../mouse/open-questions.md) own the exact cat action,
  viewpoint transition, final words as heard by Mouse, and her sensory
  experience of the incoming Big Bang.

## Open questions: Ruth Hale


- Exact mathematical career.
- Exact diagnosis, progression, and pace of decline.
- What makes her encounters feel like return without turning dementia into
  privileged cosmological access.
- How she responds to Evan's concordance window and whether she consents to
  another run.
- Her relationship with Evan's former wife.
- Exact foreshadowing and progression of her worsening fatigue, mobility,
  eating, and swallowing before terminal lucidity.
- Whether she knowingly contributes an insight that later becomes part of
  genesis.

### Related cross-subject questions

- [Plot questions](../../../work/open-questions/plot.md) own the Tess and Ruth
  friendship sequence and final exchange.
- [Evan's questions](../evan-hale/open-questions.md) own his last meaningful
  exchange with Ruth before he leaves.

## Motif open questions


- How the miscarriage planter and dead flowers recur and pay off.
- Which later beats return to the garage couch.
- How Ruth's garden changes across her decline and after her death.
- Where and how often Tess's ferry rituals recur.
- When `Kertesz` is misheard as `Curtis`, and whether the recurrence changes
  meaning.
- How Mouse's role as the last shared external reference during loss of
  separability is seeded.
- Where Mouse's survival of the local catastrophe and apparent comfort during
  contact are established and recalled.
- Where the Podkletnov reference appears and what later payoff it receives.
- Which first tiny causal seed is planted and how its consequences surface much
  later.

### Early-source candidates

- Which, if any, of the
  [early motif candidates](../research/early-source-story-craft.md#motif-quarry)
  should recur: Prussian blue as an ordinary gate, pinned vortices as memory,
  cold apparatus and hot pulses, covariance as moral evidence, or annuli and
  loops as recurrence.

## Naming open questions


### Book-level question

- Novel title. The noncanonical
  [historical title quarry](../research/early-source-story-craft.md#historical-title-quarry)
  preserves early candidates without selecting one.

### Subject-owned naming questions

- [Tess](../../canon/characters/esther-tess-kertesz/open-questions.md) owns the
  ferry and guardian names.
- [Lena](../../canon/characters/lena-varga/open-questions.md) owns her facility,
  observatory, geography, and possible surname overlap with Kertesz.
- [The projector operation](../../canon/characters/projector-operation/open-questions.md)
  owns its in-world technical and colloquial names.

## Plot open questions


### Structure and escalation

- Final part structure: five-part, four-part, or another shape.
- Exact first deliberate outcome-selection test.
- Shape and timing of Evan's addiction.
- How the long delayed coincidence chains are seeded without making the prose
  feel mechanical.
- How much of the parent universe visibly collapses before genesis.

### Cross-character beats

- The scenes through which Tess and Ruth become friends.
- The exact final exchange between Ruth and Tess.
- Whether a small, ordinary promise should remain unresolved at the final
  rupture. The [early-source prompt](../research/early-source-story-craft.md#optional-late-emotional-question)
  is noncanonical and should be used only if it grows from the characters
  rather than functioning as a tidy cosmic payoff.

### Downstream boundary

- Define the later ingestion boundary between this Snowflake and prose
  workspace and the separate Authorbot-backed `causal-projector` repository.

### Related subject-owned questions

- [The projector operation](../../canon/characters/projector-operation/open-questions.md)
  owns the proof-to-stop implementation, later institutional carriers,
  representative cross-lab handoff, local catastrophe, and terminal protocol.
- [Mouse](../../canon/characters/mouse/open-questions.md) owns the last shared
  reference and the locked cat-viewpoint ending.

## Science open questions


### Measurement and contact

- Exact test by which the impossible turn is compared across visual and
  spatio-acoustic reports without reducing them to identical imagery or
  language.
- Exact finite-history formalism and what can be measured locally.
- Exact exposure variable, memory checks, and stopping rule for Evan's
  concordance window.
- What measurable signature distinguishes causal lineage from an unrelated
  system with the same abstract topology.
- What `effective bits of rarity` means operationally.
- How Tess's rarity is discovered without turning her into a numerical magic
  key.
- The autobiographical distinction lost in the paired loss and echo.
- The ordinary before-state and cross-modal verification of the contact
  residue.

### Apparatus

- The sub-bath cold notch's exact depth and duration. Validation must beat
  ordinary magnetic-field, calibration, thermal-contact, and magnetocaloric
  explanations.
- Parked fallback: phase collapse between spatially separated diode
  thermometers if no impossible-temperature signature holds up. Do not combine
  both anomalies without a later story need.
- Which material defect and controller timing failure implement the accepted
  coherent-plus-lossy phase edge. The
  [early-source option bank](../research/early-source-math-and-mechanics.md)
  is noncanonical and includes several candidates that require technical
  scrutiny.

### Formal and experimental candidates

- Whether the finite-history ontology benefits from the bounded
  [finite-code toy count](../research/early-source-math-and-mechanics.md#finite-code-toy-count),
  and what domain and notation keep it illustrative rather than falsely
  fundamental.
- Whether target specificity has a measurable support cost distinct from raw
  improbability, feedback depth, and causal reach.
- Whether effective covariance rank versus cumulative consequential cycles can
  supply part of the exposure variable without implying that lost support
  rebounds.
- Whether full-loop reversal and controlled material substitution make
  predictions distinct enough from the accepted mapping-reversal and
  replacement-ring tests to earn story space.

### Replication implementation

- Exact custody, characterization, fabrication, and transport procedures for
  the original ring and Lena's first deliberate analogue.

### Genesis model

- How branch capacity is conserved or reallocated at genesis.
- Which stable invariants might seed the child universe's laws.

### Related subject-owned questions

- [The projector operation](../../canon/characters/projector-operation/open-questions.md)
  owns the exact proof-to-stop test, cross-lab carrier implementation,
  apparatus-linked catastrophe, and terminal protocol.
- [Mouse](../../canon/characters/mouse/open-questions.md) owns the animal,
  sensory execution of the genesis chapter's ending.
